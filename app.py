...
def generate_plots(N, mu, sigma2, S):
    ...
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title(f'Linear Regression: y = {slope.item():.2f}x + {intercept.item():.2f}')
    plt.show()
    ...
    plt.axvline(slope.item(), color="blue", linestyle="--", linewidth=1, label=f"Slope: {slope.item():.2f}")
    plt.axvline(intercept.item(), color="orange", linestyle="--", linewidth=1, label=f"Intercept: {intercept.item():.2f}")
...
def index():
    if request.method == "POST":
        ...
        slope_extreme_value = float(slope_extreme) if isinstance(slope_extreme, np.ndarray) else slope_extreme
        intercept_extreme_value = float(intercept_extreme) if isinstance(intercept_extreme, np.ndarray) else intercept_extreme

        return render_template(
            "index.html",
            plot1=plot1,
            plot2=plot2,
            slope_extreme=slope_extreme_value,
            intercept_extreme=intercept_extreme_value
        )
